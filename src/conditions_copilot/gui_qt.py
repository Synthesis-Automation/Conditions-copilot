...
from .llm_prompting.client import call_openai, DEFAULT_TEST_MODEL, DEFAULT_TEST_BASE_URL
...
class ChatWindow(QMainWindow):
    ...
    def _refresh_status(self):
        if self.llm_cmd:
            self.lbl_llm.setText("LLM: external command")
        else:
    ...
    def set_llm_cmd(self):
        ...
        if ok:
            ...
            self._refresh_status()
            if self.llm_cmd:
                self._append("INFO", "LLM: external command")
            else:
                model = os.environ.get("OPENAI_MODEL") or os.environ.get("DASHSCOPE_MODEL") or DEFAULT_TEST_MODEL
                base_url = (
                    os.environ.get("OPENAI_BASE_URL")
                ...
                self._append("INFO", f"LLM: API {model} @ {base_url}")
